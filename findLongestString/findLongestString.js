...
*/
function findLongestString(str, words) {
    ...
    charArr.forEach((char, ind) => {
        if (!alphabetMap.has(char)) {
            ...
            alphabetMap.set(char, [...alphabetMap.get(char), ind]);
        }
    });
    ...
    const checkWord = (word) => {
        ...
        for (let wordInd = 0; wordInd < word.length; wordInd++) {
            ...
            if (alphabetMap.has(letter)) {
                const alphabetInds = alphabetMap.get(letter);
                // ? use binary serach instead of indexOf
                console.log('letter:', letter);
                console.log('letterInds:', alphabetInds);
                console.log('wordInd:', wordInd);
                const found = alphabetInds.find((alphaInd) => alphaInd >= wordInd);
                // console.log('letterInds.indexOf(wordInd):', letterInds.indexOf(wordInd));
                // console.log('letterInds[letterInds.indexOf(wordInd)]:', letterInds[letterInds.indexOf(wordInd)]);
                console.log('found:', found);
                if (found === undefined) {
                    console.log('skipped letter:', letter);
                    return emptyWord;
                }
            }
            else {
